...
trie build_trie(const vector<string> & patterns) {
    trie t(1, map<char, pair<int, bool>>());
    int i, count = 0;
    for (const auto &pattern: patterns) {
        ...
        for (int j = 0; j < pattern.size(); j++) {
            ...
            if (f_r != t[i].end()) {
                if (j == pattern.size() - 1) {
                    t[i][c] = {f_r->second.first, true};
                }
                i = f_r->second.first;
            } else {
                t[i].insert({c, {count + 1, (j == pattern.size() - 1)}});
                count++;
                i = count;
                t.push_back(map<char, pair<int, bool>>());
            }
        }
    }
    return t;
...
void prefix_matching(const trie &t, const string &text, const int start, vector<int> &result) {
    ...
    while (true) {
        ...
        if (t[v].empty()) {
            ...
            if (find_result->second.second) {
                result.push_back(start);
                return;
            }
            symbol++;
            v = find_result->second.first;
        } else {
            return;
        }
    }
